...
#SPDX-FileCopyrightText: 2021 The Matrix.org Foundation C.I.C.
...
class OidcProvider:
    ...
    async def _load_metadata(self) -> OpenIDProviderMetadata:
        ...
        # ECLIPSE PATCH        
        if self._config.jwks_verify:
            metadata["jwks_verify"] = self._config.jwks_verify
    ...
    async def _parse_id_token(self, token: Token, nonce: str) -> CodeIDToken:
        ...
        # ECLIPSE PATCH
        verify = metadata.get("jwks_verify")
        claims = None
        if verify:
        ...
        async def grandfather_existing_users() -> Optional[str]:
            if self._allow_existing_users:
                ...
                if users:
                    ...
                    if len(users) == 1:
                        # ECLIPSE PATCH
                        # test over email and ask for new mxid if email already match
                        # TODO iterate over all emails
                        
                        existing_user_id = await self._store.get_user_id_by_threepid(
